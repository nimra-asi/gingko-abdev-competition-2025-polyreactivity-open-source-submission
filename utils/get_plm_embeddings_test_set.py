...
!pip install -r /Workspace/Users/nimra.asi.ext@boehringer-ingelheim.com/gingko-abdev-competition/data/precomputed_embeddings/requirements_embeddings.txt
...
print(emb_df.shape)
cols = ["esm2_cls_" + str(x) for x in emb_df.columns]
# print(cols)
emb_df.columns = cols

de = pd.concat([df[["antibody_name"]], emb_df], axis=1)
print(de.shape)
de.head(2)
...
# de.to_parquet('test_set_esm2_mean_no_sep_embeddings.parquet', engine='pyarrow', index=False)
de.to_parquet('test_set_esm2_cls_embeddings.parquet', engine='pyarrow', index=False)
...
de.to_parquet('test_set/test_set_ablang2_embeddings.parquet', engine='pyarrow', index=False)
